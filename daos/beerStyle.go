...
func (dao *BeerStyleDAO) Get(rs app.RequestScope, id int) (*models.BeerStyle, error) {
...
func (dao *BeerStyleDAO) Update(rs app.RequestScope, id int, beerStyle *models.BeerStyle) error {
	return rs.DB().Save(beerStyle).Error
}

// Delete deletes a beerStyle with the specified ID from the database.
func (dao *BeerStyleDAO) Delete(rs app.RequestScope, id int) error {
...
func (dao *BeerStyleDAO) Count(rs app.RequestScope) (int, error) {
	var count int
	err := rs.DB().Table("beerStyle").Count(&count).Error
	return count, err
}
